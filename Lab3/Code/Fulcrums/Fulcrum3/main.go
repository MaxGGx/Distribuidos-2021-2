...
func (s *server ) Intercambio (ctx context.Context, req *pb.Mensaje) (*pb.Mensaje, error) {
	ans := ""
	fmt.Println("Fulcrum 3 recibió el siguiente mensaje: "+ req.Body)
...
func processInformante(comando string)(respuesta string){
	...
	if(tipoC == "AddCity"){
		...
		//UpdateName Planeta Nombre1 Nombre2
		cP := strings.Split(comando, " ")
...
func timer(){
	...
	for i := range c {
    	...
    	for i:= range toSend1{
    		//MERGEU,<planeta>,<linea a cambiar>
    		//Se envia cada linea del archivo para que el fulcrum correspondiente actualice la info y cree las ciudades/planetas en caso de no tenerlas.
    		if _,err := os.Stat("planetas/"+toSend1[i]+".txt"); err == nil {
    			file, err := os.Open("planetas/"+toSend1[i]+".txt")
    			if err!=nil{
    				log.Fatalf("Fallo en abrir archivo")
    			}
    			scanner := bufio.NewScanner(file)
    		...
    		//MERGECLK,<planeta>,<clock a updatear>
    		for j:=range clkPlanets{
    			if(clkPlanets[j].planeta == toSend1[i]){
    				_ = enviarMsg(ipFulcrum1, "MERGECLK,"+toSend1[i]+","+(strconv.Itoa(clkPlanets[j].relojx))+" "+(strconv.Itoa(clkPlanets[j].relojy))+" "+(strconv.Itoa(clkPlanets[j].relojz)))
    				break;
    			}
    		}
    	}
